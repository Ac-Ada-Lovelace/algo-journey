// https://www.luogu.com.cn/problem/solution/UVA690
/* *uva690. 暴力搜索版本
 *TLE版本 + 小剪枝 = 过了?
 */
#include <cstdio>
...
bool check(
    int cur) // 检查该个时间片内能不能放指令, 可以使用二进制位表示用到的单元
{
...
void dfs(int d, int cur) {
    if (d == maxd + 1) {
        best = min(cur - 1, best); /*printf("best:%d\n",best);*/
        return;
    }
    for (int i = cur; i <= best; ++i) {
        if (i + (maxd - d) * minstep >= best)
            return; // 小剪枝，
        if (!check(i))
            continue;
        go(i, d);
...
int main() {
    //    freopen("in","r",stdin);
    freopen("uva690.in", "r", stdin);
    freopen("uva690.out", "w", stdout);
    while (scanf("%d", &n) == 1 && n) {
        getchar();
