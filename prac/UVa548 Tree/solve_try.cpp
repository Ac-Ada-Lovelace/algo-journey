...
tree_node* build(const vector<int>& inorder, int in_start, int in_end,
    ...
    root->left =
        build(inorder, in_start, ino_root_index - 1, postorder, post_start,
              post_start + left_tree_size - 1, ino_index_map);
    root->right =
        build(inorder, ino_root_index + 1, in_end, postorder,
              post_start + left_tree_size, post_end - 1, ino_index_map);
...
int main() {
    ...
    while (getline(cin, line)) {
        if (ino_str.empty()) {
            ...
            tree_node* root = build(inorder, 0, inorder.size() - 1, postorder,
                                    0, postorder.size() - 1, ino_index_map);
