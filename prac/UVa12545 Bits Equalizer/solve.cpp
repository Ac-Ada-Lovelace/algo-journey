...
int main() {
    ...
    for (int t = 1; t <= T; t++) {
        ...
        // Step 1: Swap '1's that need to become '0' with '0's that need to
        // become '1' This is optimal because one swap fixes two positions
        int swaps = min(A, B);
        ans += swaps;
        A -= swaps;
        B -= swaps;
        ...
        // Step 4: All remaining ?'s need one operation each (either ?->0 or
        // ?->1)
        ans += C + D;
