...
1. “讲题面”代表要尽可能根据题目文件夹内的 PDF 或参考资料逐句讲解；若缺少本地资料，可联网查询补充。对于叙述性，背景性的问题，可以逐字翻译，对于一些文化性的问题，可以适当补充背景知识；讲解时务必明确交代题目的整体流程、输入结构（字段含义、范围、顺序）与输出要求（格式、含义），必要时可引用样例说明。涉及样例时，先逐字重 Sample Input/Output，再跟随文字解释；若样例含多个 case，应分隔逐个说明。(重点：对输入的数据保证要着重说明)
